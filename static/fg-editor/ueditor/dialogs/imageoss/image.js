...
(function () {
        ...
        dialog.onok = function () {
            ...
            switch (id) {
                ...
                case 'upload':
                    ...
                    if (uploadImage.ossrenImage) {
                        $('.info', '#queueList').html('<span style="color:red;">正在渲染..</span>');
                        return false;
                    }
                    break;
                case 'online':
    ...
    // 获取fileId
    function confirmOss(data, fn) {
        $.ajax({
            url: apiRoot + 'confirm-upload-file',
            type: 'POST',
            dataType: 'json',
    ...
    // 获取文件地址
    function getFile(data, fn) {
        $.ajax({
            url: apiRoot + 'pre-get-file',
            type: 'POST',
            dataType: 'json',
            contentType: 'application/json; charset=utf-8',
            headers: {
    ...
    UploadImage.prototype = {
        ...
        initUploader: function () {
            ...
            uploader.on('all', function (type, files) {
                ...
                switch (type) {
                    ...
                    case 'uploadStart':
                        ...
                        var fileKey = +new Date() + parseInt(10 * Math.random()) + '_' + files.name;
                        files.key = oss.option.os.dir + '/' + fileKey;
                        uploader.option('formData', {
                            key: files.key,
                            OSSAccessKeyId: oss.option.os.accessid,
                            callback: '',
                            policy: oss.option.os.policy,
                            signature: oss.option.os.signature,
            ...
            uploader.on('uploadSuccess', function (file, ret) {
                var $file = $('#' + file.id);
                if (ret._raw === '') { // 针对阿里oss上传无返回情况
                    var confirmParams = [{
                      moduleId: $.cookie('moduleId') || localStorage.getItem('moduleId'),
                      orgFileName: file.name,
                      postName: file.key,
                      size: file.size,
                      needTransferFormat: 'N'
                    }]
                    confirmOss(confirmParams, function(res){
                        var getFileParams = {
                          fileId: res[0].fileId
                          // queryPara: {
                          //   'x-oss-process': 'image/resize,s_146/crop,w_146,h_146'
                          // }
                        }
                        getFile(getFileParams, function(res) {
                            _this.imageList.push({
                                fileId: res.fileId,
                                url: res.signedUrl,
                                original: file.name
                            });
                            uploadImage.ossrenImage = false;
